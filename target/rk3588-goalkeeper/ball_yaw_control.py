"""Pixel-space ball yaw controller and bounded HOST test session."""
...
class BallYawConfig:
    """Tunable pixel controller values for the Phase 5A test."""
    ...
    def validate(self):
        ...
        if not 0 <= self.min_wz <= self.max_wz <= 1000:
            raise ValueError("wz limits must satisfy 0 <= min <= max <= 1000")
...
class BallYawDecision:
    ...
    @property
    def has_target(self):
        return self.state in (BallYawState.CENTERED, BallYawState.TRACKING)


class BallYawController:
    """Convert horizontal ball position into a bounded yaw command."""

    def __init__(self, config=None):
        self.config = config or BallYawConfig()
    ...
    def update(self, x, confidence, frame_width):
        ...
        if x is None or confidence is None:
            return self._stop_decision(BallYawState.NO_TARGET, x, confidence)
        if not math.isfinite(float(x)) or not math.isfinite(float(confidence)):
            return self._stop_decision(BallYawState.NO_TARGET, None, None)
        if confidence < self.config.min_confidence:
            return self._stop_decision(
                BallYawState.LOW_CONFIDENCE, int(x), float(confidence))

        error = (float(x) - frame_width / 2.0) / (frame_width / 2.0)
        error = max(-1.0, min(1.0, error))
        if abs(error) <= self.config.deadband:
            self._last_wz = 0
            return BallYawDecision(
                BallYawState.CENTERED, error, 0, int(x), float(confidence))

        magnitude = int(round(abs(error) * self.config.proportional_gain))
        magnitude = max(self.config.min_wz, min(self.config.max_wz, magnitude))
        target = (1 if error > 0 else -1) * self.config.yaw_sign * magnitude

        # A sign reversal crosses zero first instead of briefly turning away.
        if self._last_wz != 0 and target * self._last_wz < 0:
            output = 0
        else:
            lower = self._last_wz - self.config.max_step
            upper = self._last_wz + self.config.max_step
            output = max(lower, min(upper, target))

        self._last_wz = int(output)
        return BallYawDecision(
            BallYawState.TRACKING, error, self._last_wz,
            int(x), float(confidence))

    def _stop_decision(self, state, x, confidence):
        self._last_wz = 0
        return BallYawDecision(state, 0.0, 0, x, confidence)


class BallYawSession:
    """Run a dry or explicitly armed 20 Hz ball-yaw session."""
    ...
    def tick(self, observation, frame_width, now=None):
        ...
        try:
            ...
            # Require the same consecutive confirmation after a brief loss.
            wz = (decision.wz
                  if self._acquire_count >= self.acquire_cycles else 0)
            self.link.set_twist(0, 0, wz)
