...
- 原理图/注释称 KEY1 为 PA8，但商家 GPIO 初始化又把 PA8 配为输出。实测当前输入上拉、低电平按下配置可正常切换。
...
- 到货已接好的 6P 接口可直接通信；KEY1 长按 2 秒可进入和退出 PS2 模式。
- 手柄上电后需短按 MODE，使红色 MODE 灯常亮并进入模拟模式。
- PB13 在调试模式熄灭、未解锁时闪烁、收到 3 个有效中位帧后常亮，响应符合设计。
- 架空测试中 L1/R1 dead-man、左摇杆前后/横移和右摇杆旋转均可正确驱动车轮。
- 手柄关机后接收器仍返回可被接受的模拟帧，PB13 保持常亮；150 ms 超时目前只覆盖 MCU 与 6P 接收器之间的通信中断。

尚未验证退出 PS2 后的 SWD 重连、接收器物理断开停车，以及手柄关机时接收器返回的是中位帧还是末值帧。
