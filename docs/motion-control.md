...
尚未逐轮核对 ID 或量化最低可靠速度。上电只发送广播停车。
USART1/CH340 HOST 三轴、两组 45°斜移、显式 ARM 和进程强杀后的 200 ms 停车已架空实测；HOST/PS2 互斥仍待补。
...
| `c5_host_protocol` | HOST 固定短帧解析和状态编码 |
...
- 当前镜像已通过核心板排针烧录并正常启动。
- PS2 dead-man 下的前后、横移和旋转方向均正确；充电后架空及整车运动正常。
- Orange Pi HOST 下 `vx/vy/wz=100` 方向正确；`vx=50,vy=50` 仅左前/右后转，`vx=50,vy=-50` 仅右前/左后转。
- HOST 发送进程经 `SIGKILL` 强制终止后，轮组可见自动停车，随后状态为 `DISARMED/STOPPED`。
- 单节 14500 仅约 2.3–2.5 V 时出现指示灯响应但电机不转；充电后恢复，根因是动力电压不足。
- 手柄关机后无线接收器仍向 MCU 返回合法模拟帧；当前 150 ms 超时不能代表无线链路存活。
